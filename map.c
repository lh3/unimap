...
{
	mv->n = 0;
	mm_sketch(km, seq, qlen, mi->w, mi->k, 0, mi->flag&MM_I_HPC, mv, mi->dh);
}
...
static void chain_post(const mm_mapopt_t *opt, int max_chain_gap_ref, const mm_idx_t *mi, void *km, int qlen, int *n_regs, mm_reg1_t *regs, mm128_t *a)
{
...
mm_reg1_t *mm_map_seq(const mm_idx_t *mi, int qlen, const char *seq, int *n_regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const char *qname)
{
	int i, j, rep_len, n_regs0, n_mini_pos;
	...
	*n_regs = 0;
	...
	a = mm_chain_dp(max_chain_gap_ref, max_chain_gap_qry, opt->bw, opt->max_chain_skip, opt->max_chain_iter, opt->min_cnt, opt->min_chain_score, opt->chain_gap_scale, is_splice, n_a, a, &n_regs0, &u, b->km);

	b->rep_len = rep_len;

	regs0 = mm_gen_regs(b->km, hash, qlen, n_regs0, u, a);
	if (mi->n_alt) {
	...
	chain_post(opt, max_chain_gap_ref, mi, b->km, qlen, &n_regs0, regs0, a);
	mm_est_err(mi, qlen, n_regs0, regs0, a, n_mini_pos, mini_pos);
	...
	if (b->km) {
		...
		if (mm_dbg_flag & MM_DBG_PRINT_QNAME)
			fprintf(stderr, "QM\t%s\t%d\tcap=%ld,nCore=%ld,largest=%ld\n", qname, qlen, kmst.capacity, kmst.n_cores, kmst.largest);
...
static void worker_for(void *_data, long i, int tid) // kt_for() callback
{
	...
	s->reg[i] = mm_map_seq(s->p->mi, s->seq[i].l_seq, s->seq[i].seq, &s->n_reg[i], b, s->p->opt, s->seq[i].name);
	s->rep_len[i] = b->rep_len;
}
...
static void *worker_pipeline(void *shared, int step, void *in)
{
    ...
    if (step == 0) { // step 0: read sequences
		...
		if (s->seq) {
			...
			s->n_reg = (int*)calloc(2 * s->n_seq, sizeof(int));
			s->rep_len = s->n_reg + s->n_seq;
		...
		for (i = 0; i < s->n_seq; ++i) {
			...
			if (s->n_reg[i] > 0) { // the query has at least one hit
				...
				if (p->opt->flag & MM_F_OUT_SAM)
					mm_write_sam(&p->str, mi, t, -1, 0, 0, km, p->opt->flag, s->rep_len[i]);
				else
					mm_write_paf(&p->str, mi, t, 0, 0, p->opt->flag, s->rep_len[i]);
				mm_err_puts(p->str.s);
			}
		...
		free(s->reg); free(s->n_reg); free(s->seq); // rep_len was allocated with reg; no memory leak here
		km_destroy(km);
		if (mm_verbose >= 3)
