...
static void chain_post(const mm_mapopt_t *opt, int max_chain_gap_ref, const mm_idx_t *mi, void *km, int qlen, int *n_regs, mm_reg1_t *regs, mm128_t *a)
{
	if (!(opt->flag & MM_F_ALL_CHAINS)) { // don't choose primary mapping(s)
		...
		mm_select_sub(km, opt->pri_ratio, opt->pri_ratio_max, mi->k*2, opt->best_n, opt->best_n_max, n_regs, regs);
	}
}
...
mm_reg1_t *mm_map_seq(const mm_idx_t *mi, int qlen, const char *seq, int *n_regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const char *qname)
{
	...
	*n_regs = 0;
	...
	if (opt->flag & MM_F_NO_RMQ) {
		a = mg_lchain_dp(max_chain_gap_ref, max_chain_gap_qry, opt->bw, opt->max_chain_skip, opt->max_chain_iter, opt->min_cnt, opt->min_chain_score,
						 opt->chain_gap_scale * .2, 0.0, is_splice, n_a, a, &n_regs0, &u, b->km);
	} else {
