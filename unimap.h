...
#define UM_VERSION "0.0-r37-dirty"
...
typedef struct {
	...
	uint32_t mapq:8, split:2, rev:1, inv:1, sam_pri:1, split_inv:1, is_alt:1, dummy:17;
	uint32_t hash;
	float div;
	mm_extra_t *p;
} mm_reg1_t;
...
typedef struct {
	...
	float end_len_frac, gap_flank_frac;

	float mask_level;
	int mask_len;
	float pri_ratio, pri_ratio_max;
 ...
 */
mm_reg1_t *mm_map_seq(const mm_idx_t *mi, int l_seq, const char *seq, int *n_regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const char *name);
